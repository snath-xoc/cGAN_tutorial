...
def get_dates(year,
    ...
    end_date = datetime.date(year+1, 1, 2)  # go a bit into following year
...
def load_hires_constants(batch_size=1):
    ...
    # LSM is already 0:1
    lsm = df["lsm"].values
    lsm = interp_to_rfe(lats,lons,lsm)
    df.close()
...
def load_fcst(field,
    ...
    lats = nc_file["latitude"][:]
    lons = nc_file["longitude"][:]
